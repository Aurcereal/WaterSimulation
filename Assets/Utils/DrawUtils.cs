...
public static class DrawUtils {
    ...
    public static void DrawCircle(float3 pos, float3 euler, float radius, Color color, int layer = 1) {
        Matrix4x4 transform = Matrix4x4.Translate((Vector3) pos) * Matrix4x4.Rotate(Quaternion.Euler((Vector3) euler)) * Matrix4x4.Scale(Vector3.one * radius);
        circleMaterial.SetColor("_Color", color);
        Graphics.DrawMesh(MeshUtils.QuadMesh, transform, circleMaterial, layer);
    }
    ...
    static void DrawOutlineBox(float3 pos, float3 euler, float2 dim, float thickness, Color color, int layer = 1) {
        Matrix4x4 transform = Matrix4x4.Translate((Vector3) pos) * Matrix4x4.Rotate(Quaternion.Euler((Vector3) euler)) * Matrix4x4.Scale(new Vector3(dim.x*0.5f, dim.y*0.5f, 1.0f));
        outlineBoxMaterial.SetColor("_Color", color);
        outlineBoxMaterial.SetVector("_Dim", (Vector2) dim);
        outlineBoxMaterial.SetFloat("_Thickness", thickness);
        Graphics.DrawMesh(MeshUtils.QuadMesh, transform, outlineBoxMaterial, layer);
    }
